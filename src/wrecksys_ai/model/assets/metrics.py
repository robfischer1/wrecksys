...
def metrics_list(top_k: List[int]) -> List[keras.metrics]:
    global_metrics = [GlobalRecall(name=f'Global_Recall/Recall_{k}', top_k=k) for k in top_k]
    # global_metrics = []
    # global_metrics.append(GlobalMeanRank(name='Global_Mean_Rank'))
    batch_metrics = [BatchRecall(name=f'Batch_Recall/Recall_{k}', top_k=k) for k in top_k]
